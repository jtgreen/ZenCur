...
class experiment:
    def __init__(self):
        ...
        self.kvcap = 2.1*math.pow(10,-3) * 1000/60 # JTG: ZenCur paper in L/min/mmHg; the Zenker paper in mL/s/mmHg
        self.Pcvp0 = 18.0981 # First pig's parameter

        # Experiment 
        # fluidChangeRes

        fluidVolume = 1560 # % resuscitation volume JTG: + bleed volume; however now just bleed volume

        self.startTime = 2400 # JTG: First pig start time
        self.endTime = 4380 # JTG: 33 min in seconds
        self.startTimeRes = 2000 # Volume resucitation (not used in zencur, but here if you want it)
        self.endTimeRes = 3000
        self.totalVolume = fluidVolume

        # total experiment time
        self.tspan = [0, 30000] # JTG: Time of ZenCur experiment
        ...
        totalVolume = 4290 # JTG: First pig's circulating initial volume
    ...
    # ZenCur addition
    def wellWithin(self, t, startTime, endTime, startTimeRes, endTimeRes, totalVolume, Pcvp0, Pcvp):
        # % input function for model to simulate fluid resuscitation/withdrawal
        if (t >= startTime and t < endTime):
            return -totalVolume/(endTime-startTime) + self.kvcap*(Pcvp0 - Pcvp) # dVdt, also, JTG: cap refill
        # elif (t >= startTimeRes and t < endTimeRes):
        #     return totalVolume/(endTimeRes-startTimeRes) # dVdt
        elif t >=endTime:
            #import ipdb; ipdb.set_trace()
            # cap refill
            return self.kvcap*(Pcvp0 - Pcvp)
        else:
            return 0 # dVdt
    ...
    def odes(self, y, t):
        ...
        Pv1 = self.Pv(Vv, self.Cv, Vv_relaxed)  #JTG: Pcvp
        ...
        #if t>self.endTime:
        #    import ipdb; ipdb.set_trace()
        dVv =  -dVa + self.wellWithin(t, self.startTime, self.endTime, 
                    self.startTimeRes, self.endTimeRes, self.totalVolume, self.Pcvp0, Pv1) #% external volume substitution/withdrawal affects venous compartment
